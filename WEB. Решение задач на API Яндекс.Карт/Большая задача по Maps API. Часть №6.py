...
class RESP:
    ...
    def get(self):
        map_request = f"http://static-maps.yandex.ru/1.x/?ll={self.x},{self.y}&spn={self.loc},{self.loc}&l={self.type_map}&geocode={self.poisk_string}&pt={'~'.join(map(str, self.points))}"
        print(map_request)
        response = requests.get(map_request)
        if not response:
    ...
    def get_lock(self, delta_loc):
        ...
        if delta_loc == -1:
            self.loc /= self.delta
            self.x_delta /= self.delta
            self.y_delta /= self.delta
        elif delta_loc == 1:
            self.loc *= self.delta
            self.x_delta *= self.delta
            self.y_delta *= self.delta
    ...
    def append_point(self):
        self.points.append(Point(self.x, self.y, len(self.points) + 1))
