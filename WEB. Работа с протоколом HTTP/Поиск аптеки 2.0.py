...
def main():
    ...
    param = f"pt={org_lat},{org_lon},pm2dgl"
    show_map(f"ll={address_ll}&spn={span}", "map", add_params=param)
    params = param + f"~{address_ll},pm2rdl"
    show_map("ll={0}&spn={1}".format(address_ll, span), "map", add_params=params)
    show_map(map_type="map", add_params=params)
