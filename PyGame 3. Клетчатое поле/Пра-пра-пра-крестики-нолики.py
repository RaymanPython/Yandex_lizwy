...
class Board:
    ...
    def zerol(self, screen, x, y, size):
        pygame.draw.circle(screen, pygame.Color(255, 0, 0), (x + size // 2, y + size // 2), size // 2, 2)
        ...
        for y in range(self.height):
            for x in range(self.width):
                ...
                if self.color_matric[x][y] == 1:
                    self.krest(screen, x * self.cell_size + self.left + 1, y * self.cell_size + self.top + 1,
                               self.cell_size)
                elif self.color_matric[x][y] == 2:
                    self.zerol(screen, x * self.cell_size + self.left + 1, y * self.cell_size + self.top + 1,
                               self.cell_size)
    ...
    def remove_color(self, pos):
        ...
        if pos != None:
            ...
            x -= 1
            y -= 1
            if self.color_matric[x][y] == 0:
                if self.xod:
                    self.color_matric[x][y] = 1
                else:
                    self.color_matric[x][y] = 2
